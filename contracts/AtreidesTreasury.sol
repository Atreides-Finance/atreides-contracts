...
contract AtreidesTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSPICE }
    ...
    address public sSPICE;
    uint public sSPICEQueue; // Delays change to sSPICE address
    ...
    /**
        @notice allow approved address to deposit an asset for SPICE
        @param _amount uint
    ...
    /**
        @notice allow approved address to burn SPICE for reserves
        @param _amount uint
        @param _token address
     */
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSPICE ).balanceOf( msg.sender ); // Can only borrow against sSPICE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SPICE
        @param _amount uint
     */
    function repayDebtWithSPICE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISPICEERC20( SPICE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SPICE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SPICE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SPICE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SPICE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SPICE decimals
            value_ = _amount.mul( 10 ** IERC20( SPICE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSPICE ) { // 9
            sSPICEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
