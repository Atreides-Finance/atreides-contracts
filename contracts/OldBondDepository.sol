...
contract AtreidesSpiceBondDepository is Ownable {
    ...
    address public immutable SPICE; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints SPICE when receives principle
    ...
    struct Bond {
        uint256 payout; // SPICE remaining to be paid
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) SPICE
         */
        ...
        if (fee != 0) {
            ...
            IERC20(SPICE).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(SPICE).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(SPICE).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(SPICE).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(SPICE).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SPICE supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(SPICE).totalSupply();
    ...
    /**
     *  @notice calculate amount of SPICE available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or SPICE) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != SPICE);
