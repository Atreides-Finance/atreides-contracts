...
contract USDT is LibNote {
    ...
    modifier auth() {
        require(wards[msg.sender] == 1, "USDT/not-authorized");
        _;
    }
    ...
    mapping(address => uint256) public usdtMintedToday;
    ...
    constructor(uint256 chainId_) {
        ...
        dailyUSDTLimit = 10000000000000000000000;
    }
    ...
    function adjustdailyUSDTLimit(uint256 _limit) external auth {
        dailyUSDTLimit = _limit;
    }
    ...
    function mint(address usr, uint256 wad) external {
        if (wards[msg.sender] == 0) {
            ...
            if (sub(block.number, lastMintRestart[msg.sender]) >= 6500) {
                ...
                usdtMintedToday[msg.sender] = add(
                    usdtMintedToday[msg.sender],
                    wad
                );
            }
        }
    ...
    function burn(address usr, uint256 wad) external {
        require(balanceOf[usr] >= wad, "USDT/insufficient-balance");
    ...
    function permit(
        ...
        require(holder != address(0), "USDT/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "USDT/invalid-permit");
        require(
            expiry == 0 || block.timestamp <= expiry,
            "USDT/permit-expired"
        );
        require(nonce == nonces[holder]++, "USDT/invalid-nonce");
