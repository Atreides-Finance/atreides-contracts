...
contract BondingCalculator is IBondingCalculator {
    ...
    function getKValue(address _pair) public view returns (uint256 k_) {
        ...
        uint256 decimals = token0.add(token1);
        uint256 pair = IERC20(_pair).decimals();
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(_pair)
            .getReserves();
        if (decimals >= pair)
            k_ = reserve0.mul(reserve1).div(10**decimals.sub(pair));
        else k_ = reserve0.mul(reserve1).mul(10**pair.sub(decimals));
    }
    ...
    function markdown(address _pair) external view returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == SPICE) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return
            reserve.mul(2 * (10**IERC20(SPICE).decimals())).div(
                getTotalValue(_pair)
            );
    }
}
